...
[`vendor/sc64/tests/gif`](../gif/README.md). The maintained
results live in [SC64 UI testing](../../docs/phosphoros-research/ui-testing.md). This directory
owns no USB protocol implementation; it uses the existing `usb_send.py` CLI over
`sc64usb.py`.

## Reproduce the fixture

Install the developer dependencies `numpy` and `pillow`. From the PhosphorOS checkout root containing this SummerCart64 submodule, supply the upstream original Final Fight GIF and an ignored output path:

```sh
python vendor/sc64/tests/phosphoros-receiver/fixtures.py \
  "/path/to/Final Fight/images/background.gif" \
  build/n64-sc64/test/sc64-gif
python vendor/sc64/tests/phosphoros-receiver/check_fixture.py \
...
```sh
python vendor/sc64/tests/phosphoros-receiver/inject.py \
...
```sh
python vendor/sc64/tests/phosphoros-receiver/rtl_fixtures.py \
  "/path/to/Final Fight/images/background.gif" \
