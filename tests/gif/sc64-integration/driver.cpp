...
struct Rig {
    ...
    bool tick() {
        ...
        if(!prior_request&&t.arb_request) {
            ...
            if(expected_source==1)assert(t.arb_address<0x30000);
    ...
    void reset() {
        ...
        t.n64_request=0;t.usb_request=0;t.sd_request=0; t.reset=1; t.cancel=0;t.session_start=0;t.frame_start=0;
    ...
    void frame(uint32_t id,const std::vector<uint8_t> &data,const std::vector<uint8_t> &expected,unsigned minimum,bool lzw) {
        ...
        size_t at=0;bool complete=false,decoded=!lzw;uint64_t limit=cycles+20000000;
...
int main(int argc,char **argv) {
    ...
    ledger<<"frame,cycles,packet_bytes,sdram_reads,sdram_writes\n";
    for(unsigned id=0;id<count;id++) {
        ...
        uint64_t before=r.cycles,prior_bytes=r.packet_bytes,prior_reads=r.reads,prior_writes=r.writes;r.frame(id,data,expected,minimum,true);
        if((id%4)==3||id+1==count)r.emit(id);
        ledger<<id<<","<<r.cycles-before<<","<<r.packet_bytes-prior_bytes<<","<<r.reads-prior_reads<<","<<r.writes-prior_writes<<"\n";assert(ledger);
