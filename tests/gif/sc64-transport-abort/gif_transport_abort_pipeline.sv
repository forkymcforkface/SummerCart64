...
module gif_transport_abort_pipeline (
    ...
    output logic aborting, retired, adapter_busy, mux_idle, arb_write,
    ...
    logic cancel_seen;
    wire cancel_pulse=cancel&&!cancel_seen&&!aborting;
    wire admit=!aborting&&!cancel;
    wire frame_accept=frame_start&&admit;
    wire verify_accept=verify_start&&admit;
    wire emit_accept=emit_packet&&admit;
    ...
    gif_lzw_cached decoder (
        .clk(clk), .reset(reset), .start(frame_accept && !bypass_lzw), .cancel(cancel_pulse),
    ...
    assign in_ready = admit&&(bypass_lzw ? compose_ready : compressed_ready);
    assign pixel_ready = compose_ready && !bypass_lzw;
    assign error = compose_error || (!bypass_lzw && decode_error);
    gif_ci8_compose compositor (
        .clk(clk), .reset(reset), .session_start(session_start&&admit), .frame_start(frame_accept),
        .emit_packet(emit_accept), .packet_ack(packet_ack&&admit),
        .cancel(cancel_pulse || (!bypass_lzw && decode_done && decode_error)),
        ...
        .in_valid(admit&&(bypass_lzw ? in_valid : pixel_valid)), .in_ready(compose_ready),
    ...
    assign arb_write=sdram_bus.write;
    ...
    gif_sc64_bus adapter (
        .clk(clk),.reset(reset),.configure(configure&&admit),.cancel(cancel_pulse),
        .arena_begin(27'd0),.arena_end(27'h30000),
        .dictionary_base(27'd0),.canvas_base(27'h10000),
        .configured(configured),.config_error(config_error),.busy(adapter_busy),.cancelled(),
    ...
    wire source_pop=admit&&(verification?verify_ready:compressed_ready)&&source_count!=0;
    wire output_push=out_valid&&output_ready;
    wire output_pop=output_stream.rx_read;
    assign source_control.start=frame_accept||verify_accept;
    assign source_control.stop=cancel_pulse;
    ...
    always_ff @(posedge clk) begin
        output_start<=1'b0;retired<=1'b0;
        if(reset) begin
            aborting<=0;retired<=0;cancel_seen<=0;source_count<=0;source_head<=0;source_tail<=0;source_consumed<=0;
            ...
            if(!cancel)cancel_seen<=0;
            if(cancel_pulse)begin aborting<=1'b1;cancel_seen<=1'b1;end
            ...
            if((frame_accept||verify_accept)&&(source_control.busy||source_count!=0))$fatal(1,"source reused before retirement");
            if(emit_accept&&output_busy)$fatal(1,"output reused before retirement");
            if(frame_accept||verify_accept) begin
                source_count<=0;source_head<=0;source_tail<=0;source_consumed<=0;
                verification<=verify_accept;
                source_limit<=verify_accept?output_length:source_length;
            end else begin
            ...
            if(aborting&&!decode_busy&&!busy&&!adapter_busy&&!source_control.busy&&
               !output_control.busy&&mux_idle&&!source_bus.request&&!output_bus.request&&
               !scratch_bus.request&&!cfg_bus.request)begin
                source_count<=0;source_head<=0;source_tail<=0;
                output_count<=0;output_head<=0;output_tail<=0;
                source_consumed<=0;source_limit<=0;verification<=0;
                output_length<=0;emitted<=0;header_payload<=0;
                aborting<=0;retired<=1;
            end
        end
    end
    gif_transport_mux mux (.clk(clk),.reset(reset),.idle(mux_idle),.scratch(scratch_bus),
