...
struct Rig {
    ...
    bool tick() {
        t.clk=0;t.mem_ack=0;t.mem_error=0;t.eval();
        if(t.reset) pending=false;
            ...
            if(wait) wait--;
                ...
                t.mem_ack=1; assert(address<76800);
            ...
            pending=true;address=t.mem_address;wr=t.mem_write;value=t.mem_wdata;
            wait=unsigned(cycles%3);
        }
        t.eval();
