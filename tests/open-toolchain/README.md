...
This build verifies clean source pins, applies all three hard-cell patches, corrects
the FIFO flag routing graph, installs the corrected database, and regenerates
the 7000 nextpnr database. It also layers the verified
[ALLPORTS parser](constraints/allports/README.md) and
[stock SYSCONFIG entries](constraints/sysconfig/README.md), and installs
checksum-pinned sv2v v0.0.13 for the
...
The context excludes generated Python caches. The parser additions make the
complete original LPF parse, but do not implement its missing external timing
and exception semantics. Parser acceptance must never count as full constraint
qualification. The ALLPORTS-only runner records its intermediate parser stage;
the SYSCONFIG runner exercises the completed parser layer.

The clean parser-inclusive image is recorded locally as
`sc64-open:diagnostic-lpf`, ID
`sha256:15cb73a02b74c4adf32c87d01537fec490dc83dafe02d3720076fca1c3ea4df4`.
Independent fresh-container EFB preparation/barriers, FIFO roundtrip/negative
checks, ODDR/PLL roundtrip/negative checks, and SYSCONFIG exact-bit fixtures all
pass. Evidence: `build/sc64-open-release/image-*` and
`diagnostic-lpf-clean-build.log` in the parent workspace. No generated Python
cache is present under the image's diagnostic tools. These are diagnostic
qualification checks, not a successful full-source FPGA release build.
...
The [installed timing database probes](timing-data/README.md) identify exact
7000 hard-block records, but a minimum-delay oracle disproves direct numeric
import. The [OpenSTA fixtures](constraints/sta/README.md) pass signed-delay,
phase and annotation-coverage checks with synthetic models; they do not yet
provide MachXO2 timing. [Further synthesis probes](memory/timing-options2.md)
fail to improve the existing route baseline. The
[SCUBA frontend probe](efb/scuba/README.md) reproduces the stock EFB wrapper
and converts its EDIF to NGD without a licensed mapping step.

[Public mapped oracles](timing-data/public-oracles.md) extend the evidence to
7000HE, while [supported timing-export probes](timing-data/export-followup.md)
remain unsuccessful. The [reference coverage audit](unknown-bits-audit.md)
separates undefined bits from unmatched known fields; a
[public idle-EBR comparison](timing-data/public-ebr-audit.md) rules out one
unconditional site-field hypothesis without identifying a safe encoding.
