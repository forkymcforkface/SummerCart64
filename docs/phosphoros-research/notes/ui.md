...
[docs/perf_log.md](../../docs/perf_log.md) records real `browse10-nav` runs with music/effects. On 2026-08-25, Final Fight had 341 frames / 10 seconds with PDSA RLE reverse-baseline and 438 with PDSB LZ4 using the same ROM. Direct PDSB record DMA reached 482; later background-copy candidate reached 494. These demonstrate both decode and transport improvements, but the final historical result still misses sustained 60 FPS (p50 19 ms, p99 41 ms). They do not prove today's firmware result.
